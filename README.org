...
**** Unique Pointer
- =unique_ptr= is scoped pointer, it just emulates the stack arrays in destruction.
- =unique_ptr= is called unique because it cannot be copied as any one of pointer going out of scope would free the memory the copied group of ptrs were pointing.
- Thus =unique_ptr= is used when you want a single instance and scoped type of ptr.
...
- However, calling ~new Type();~ is not recommended for shared pointer as shared pointer has to create a control block for ref count, so calling new (creating object) then passing it to shared pointer to make that object again is inefficient.
